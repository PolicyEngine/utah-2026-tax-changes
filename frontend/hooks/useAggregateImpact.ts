...
// Utah 2026 tax changes dashboard — year is locked to 2026 throughout.
export const UTAH_DASHBOARD_YEAR = 2026;
...
// Fetch the national congressional_districts.csv and filter to Utah (state="UT").
// Shared by components that need district-level numbers for the Utah 2026
// tax changes dashboard.
export function useUtahDistrictImpact(enabled: boolean, year: number = UTAH_DASHBOARD_YEAR) {
  return useQuery<Record<string, string | number>[]>({
    queryKey: ["utahDistrictImpact", year],
    queryFn: async () => {
      const rows = await fetchCSV("congressional_districts.csv");
      return rows.filter(
        (r) => r.state === "UT" && Number(r.year) === year,
      );
    },
    enabled,
    staleTime: 5 * 60 * 1000,
